...
int SerialPort::Peek(unsigned char *buf, int len) {
  len = stream_.Peek(buf, len);
  return len;
}

int SerialPort::Read(unsigned char *buf, int len, int timeout) {
...
int SerialPort::Read(unsigned char *buf, int maxLen, unsigned char *end, int timeout, int *recvLen) {
  int endLen = sizeof(end);
  timestamp_t start = TimeStamp::now();
  int total = 0;
  while (total < maxLen) {
    mutex_.lock();
...
bool SerialPort::EndsWith(unsigned char *str, int strLen, unsigned char *end, int endLen) {
  if (strLen < endLen)
    return false;
