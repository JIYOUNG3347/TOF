...
{
  buffer_=new unsigned char[initCapacity];
...
unsigned char SerialPortStream::Peek()
{
  if(length_==0)
    return 0;
...
int SerialPortStream::Peek(unsigned char* buf,int len)
{
  if(len>length_)
    len=length_;
  for(int i=0;i<len;i++)
...
unsigned char SerialPortStream::Take()
{
  if(length_==0)
    return 0;
  unsigned char aChar=buffer_[start_];
  start_++;
  length_--;
  if(start_>=capacity_)
    start_-=capacity_;
...
int SerialPortStream::Take(unsigned char* buf,int len)
{
  if(len>length_)
    len=length_;
  for(int i=0;i<len;i++)
